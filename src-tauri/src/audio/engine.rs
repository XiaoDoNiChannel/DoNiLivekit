...
use crate::state::{AppState, MicSessionManager};
use crate::transport::pcm_websocket::{MICROPHONE_ADDRESS, PROCESS_AUDIO_ADDRESS};
...
pub(crate) async fn start_capture_multi(
    ...
    if normalized.is_empty() {
        return Err(AudioError::InvalidInput("没有可用的 PID，无法启动采集".into()).into());
    }
...
pub(crate) fn set_rust_mic_device_id(
    ...
    let mut guard = state
        ...
        .lock()
        .map_err(|_| AudioError::StatePoisoned("selected_mic_device_id").to_string())?;
...
pub(crate) fn list_capture_devices() -> Result<Vec<AudioDeviceInfo>, String> {
    Err(
        AudioError::InvalidInput("当前平台不支持 Rust 麦克风设备枚举，仅 Windows 可用".into())
            .into(),
    )
}
...
pub(crate) async fn start_audio_pump(
    ...
    let listener = match TcpListener::bind(&addr).await {
        Ok(v) => v,
        Err(e) => {
            let error = AudioError::Transport(format!("无法绑定 9001 端口: {e}"));
            println!("[audio/process_loopback] {error}");
            return;
        }
    };
            ...
            loop {
                ...
                loop {
                    ...
                    if min_available >= MIX_BLOCK_SAMPLES {
                        let mut mixed_samples = Vec::with_capacity(MIX_BLOCK_SAMPLES);
                        ...
                        let mixed_chunk = samples_to_pcm_f32le(&mixed_samples);
...
pub(crate) async fn start_mic_pump(
