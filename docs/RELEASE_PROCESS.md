...
6. 将 `latest.json` 和它引用的 Windows updater bundle同步到 DoNiChannel FastAPI 服务器的 `downloads/`（或国内对象存储）。中心服务器可直接执行：

   ```powershell
   .\scripts\Publish-LanUpdate.ps1 -SourceDirectory "D:\release\v0.2.0"
   ```

   脚本默认检查 `http://10.126.126.67:5000`。发布文件时必须先放 `.nsis.zip`，最后放 `latest.json`；`.exe` 只用于全新/手动安装。
