...
logging.basicConfig(
    level=os.environ.get("DONICHANNEL_LOG_LEVEL", "INFO").upper(),
    format="%(asctime)s %(levelname)s %(name)s %(message)s",
)
LOGGER = logging.getLogger("donichannel.server")
...
async def _start_background_tasks() -> None:
    await start_presence_cleanup_task()


async def _stop_background_tasks() -> None:
    await stop_presence_cleanup_task()


app = FastAPI(
    title="DoNiChannel Backend",
    version=SERVER_VERSION,
    lifespan=create_lifespan(
        _initialize_resources, _start_background_tasks, _stop_background_tasks
    ),
)
...
def init_db() -> None:
    """Apply forward-only migrations and preserve a recovery backup."""
    version, backup_path = migrate_database(DB_PATH, DEFAULT_ROOMS)
    LOGGER.info("action=database_ready version=%s backup=%s", version, backup_path or "none")


def get_all_rooms_from_db() -> List[str]:
    """从 SQLite 读取全部语音频道。"""
    return room_repository.list_rooms(DB_PATH)


def add_room_to_db(room_name: str) -> None:
    """新增语音频道。"""
    room_repository.add_room(DB_PATH, room_name)
...
def db_save_message(msg: dict) -> None:
    ...
    chat_repository.save_message(DB_PATH, msg, CHAT_MAX_MESSAGES_PER_CHANNEL)
...
def db_get_history(channel_id: str, limit: int = 50, before_ts: Optional[int] = None) -> List[dict]:
    ...
    """
    return chat_repository.get_history(DB_PATH, channel_id, limit, before_ts)


def db_update_reactions(message_id: str, reactions: dict) -> None:
    """更新一条消息的 reactions 字段。"""
    chat_repository.update_reactions(DB_PATH, message_id, reactions)


def db_get_message(message_id: str) -> Optional[dict]:
    """按 ID 查单条消息。"""
    return chat_repository.get_message(DB_PATH, message_id)
...
def db_get_profile(identity: str) -> Optional[dict]:
    ...
    return profile_repository.get_profile(DB_PATH, identity)
...
def build_token(user_name: str, room_name: str, identity: Optional[str] = None) -> str:
    ...
    """
    return build_room_token(
        API_KEY,
        API_SECRET,
        user_name,
        room_name or DEFAULT_ROOM_NAME,
        identity,
    )
...
async def list_livekit_rooms_and_participants() -> dict:
    ...
    """
    return await list_rooms_and_participants(LIVEKIT_URL, API_KEY, API_SECRET)
...
@app.post("/api/rooms")
async def create_room(body: RoomAction = Body(default_factory=RoomAction)):
    """创建语音频道。"""
    payload = body.model_dump(by_alias=True)
    room_name = str(payload.get("name") or payload.get("room_name") or "").strip()
...
async def save_user_profile(body: ProfileUpdate = Body(default_factory=ProfileUpdate)):
    """保存当前用户资料。

    新客户端按 userId 持久化 displayName/avatar/status，Presence 负责实时广播。
    """
    payload = body.model_dump(by_alias=True)
    user_id = str(payload.get("userId") or payload.get("identity") or "").strip()
    if not user_id:
        raise HTTPException(status_code=400, detail="userId 不能为空")

    display_name = str(payload.get("displayName") or "未命名用户").strip()[:24] or "未命名用户"
    avatar_color = str(payload.get("avatarColor") or "#5865f2").strip()[:32] or "#5865f2"
    avatar_preset = str(payload.get("avatarPreset") or "").strip()[:16]
    avatar_url = str(payload.get("avatarUrl") or "").strip()[:512]
    status_text = str(payload.get("statusText") or "在线").strip()[:32] or "在线"
...
async def presence_websocket(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            try:
                message = decode_message(raw_message, "Presence")
...
async def chat_websocket(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            try:
                message = decode_message(raw_message, "Chat")
            except ProtocolMessageError as error:
            ...
            if message_type == "ping":
                await chat_manager.send_to_connection(
                    connection_id, heartbeat_response("chat")
                )
